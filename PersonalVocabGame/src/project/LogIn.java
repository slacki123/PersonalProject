...
public class LogIn {
	...
	public static void logIn() {
		...
		while (accessGranted == false) {
			...
			try {
				...
				String fetchedUser1 = GameDatabase.fetchUsername(usernameInput);
				...
				if (usernameInput.equals(fetchedUser1)) {
					...
					String fetchedUser2 = GameDatabase.fetchUsername(input);
					...
					if (input.equalsIgnoreCase("register")) {
						...
						GameDatabase.createUser(userName, password);
					} else {
	...
	public static void logInUsernameFound(String usernameInput) {
		...
		String fetchedPassword = GameDatabase.fetchPassword(usernameInput);
		...
		if (passwordInput.equals(fetchedPassword)) {
			...
			System.out.println("Admin privileges: " + GameDatabase.fetchAdminStatus(LogIn.loggedInUsername) + "\n");
